...
_B64_KEY = base64.urlsafe_b64encode(b"12345678901234567890123456789012").decode('utf-8')
...
@pytest.mark.asyncio
async def test_unsigned_batch_rejected_or_quarantined(client: AsyncClient, setup_device, session_factory):
    payload = {
...
@pytest.mark.asyncio
async def test_verified_batch_gets_credit(client: AsyncClient, setup_device, session_factory):
    payload = {
        "batch_uuid": str(uuid4()),
        "feedstock_species": "Lantana_camara",
        "harvest_timestamp": "2026-01-01T00:00:00Z",
    ...
    op_id = "op-batch-2"
    canonical = "\n".join(["POST", "/api/v1/batches", op_id, hashlib.sha256(raw_body).hexdigest(), setup_device]).encode("utf-8")
    sig = hmac.new(b"12345678901234567890123456789012", canonical, hashlib.sha256).hexdigest()
    ...
    import uuid
    async with session_factory() as session:
        result = await session.execute(select(Batch).where(Batch.batch_uuid == uuid.UUID(payload["batch_uuid"])))
