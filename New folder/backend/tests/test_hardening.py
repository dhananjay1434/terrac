...
async def test_p0_13_batch_dup_op_id_same_payload_returns_200(client):
    op_id = str(uuid4())
    body = _valid_payload()
    r1 = await client.post("/api/v1/batches", json=body,
                           headers={"X-Idempotency-Key": op_id})
    assert r1.status_code == 201, r1.text
    r2 = await client.post("/api/v1/batches", json=body,
                           headers={"X-Idempotency-Key": op_id})
...
async def test_p0_13_batch_dup_op_id_different_sha_returns_409(client):
    ...
    r1 = await client.post("/api/v1/batches", json=body1,
                           headers={"X-Idempotency-Key": op_id})
    assert r1.status_code == 201
    r2 = await client.post("/api/v1/batches", json=body2,
                           headers={"X-Idempotency-Key": op_id})
...
async def test_p0_13_media_dup_op_id_different_hash_returns_409(client):
    ...
    r1 = await client.post(
        ...
        headers={"X-Idempotency-Key": op_id, "X-Declared-SHA256": sha_a, "X-Device-Id": "device-123", "X-Batch-UUID": str(uuid4())},
    )
    assert r1.status_code == 200, r1.text
    r2 = await client.post(
        "/api/v1/media",
        files={"file": ("b.jpg", io.BytesIO(content_b), "image/jpeg")},
        headers={"X-Idempotency-Key": op_id, "X-Declared-SHA256": sha_b, "X-Device-Id": "device-123", "X-Batch-UUID": str(uuid4())},
    )
    assert r2.status_code == 409
...
@pytest.mark.parametrize("path", ["/api/v1/telemetry", "/api/v1/yield",
                                  "/api/v1/metadata", "/api/v1/application"])
@pytest.mark.asyncio
...
async def test_p0_16_single_high_temp_rejected(client):
    """A min_temp asserted between 0 and 100 °C is suspicious — must be rejected."""
    body = _valid_payload(min_recorded_temp_c=50.0)
    r = await client.post("/api/v1/batches", json=body,
                          headers={"X-Idempotency-Key": str(uuid4())})
    assert r.status_code == 422
...
def test_p0_17_no_hardcoded_db_default():
    db_py = (Path(__file__).resolve().parents[2]
             / "uploaded" / "New folder" / "backend" / "db.py").read_text(encoding="utf-8")
    ...
    assert ('os.environ.get("DATABASE_URL",' not in db_py
...
# =============================================================================
# P1-18 — X-Mock-Location header rejected
# =============================================================================

@pytest.mark.asyncio
async def test_p1_18_mock_location_header_rejected(client):
    content = b"x"
    ...
    assert r.status_code == 403, (
        f"X-Mock-Location:true must be rejected (got {r.status_code}). "
        "See /app/detailed.md#P1-18."
    )
...
def _sign(method: str, path: str, op_id: str, body: bytes, device: str,
          secret: bytes = b"test-secret") -> str:
    canonical = "\n".join([
        method, path, op_id, hashlib.sha256(body).hexdigest(), device
    ]).encode()
    return _hmac.new(secret, canonical, hashlib.sha256).hexdigest()
...
def test_p1_21_schemas_py_deleted_or_uses_model_validator():
    schemas_py = (Path(__file__).resolve().parents[2]
                  / "uploaded" / "New folder" / "backend" / "schemas.py")
    ...
    assert re.search(r"@field_validator\(\s*[\"']biomass_sourcing[\"']", text) is None, (
