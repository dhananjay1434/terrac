# R2 — Extract `settings.py` (env loading, secrets, feature flags, constants)

> **Read `00_SERVER_REFACTOR_SOP.md` first.** Step 2 of 10. R1 must be committed & green. Pure relocation.
> Baseline gate: **416 passed, 2 skipped**. ONE commit.

**What moves:** the config layer — `_load_env`, secret resolution, the versioned-HMAC startup validation,
and the three env-live feature-flag readers. **Critical constraint (SOP §6.5):** the flag readers MUST
stay functions that read `os.environ` at call time — never freeze them into import-time constants; tests
`importlib.reload(server)` and monkeypatch env.

**One cross-dependency to resolve:** `_canonical_skew_seconds()` (a flag reader) calls `_rl_int()`, a
generic env-int helper that currently lives in the rate-limit block (moves to `middleware.py` in R8). To
avoid R2 depending on R8, we move the generic `_rl_int` into `settings.py` **now** and have the later
middleware import it from settings. `_rl_int` is a pure `os.environ.get(name, default)`-to-int helper with
no rate-limit specifics, so this is clean.

---

## STEP 1 — Create `backend/settings.py`
...
> **Note on `_canonical_skew_seconds`:** the original called `_rl_int`; here it calls `env_int` (the same
> function, renamed at the generic layer). Behavior is byte-identical. `_rl_int` remains available as an
> alias for the middleware and for any `server._rl_int` reference.
...
1. **Delete** from server.py (locate by name):
   - `_load_env` def + the `_load_env()` call (lines ~98–113)
   - `_MIN_SECRET_LEN`, `_MIN_SECRET_UNIQUE`, `_require_secret` (~196–224)
   - the `hmac_keys.validate_startup()` line, `_HMAC_SECRET`, `_ADMIN_SECRET` (~230–235)
   - `_attestation_enforced`, `_canonical_skew_seconds`, `_require_canonical_v2` (~246–261)
   - `log = logging.getLogger("dmrv")` + the two `observability.*` init calls (~264–268)
   - **Do NOT delete `_rl_int` from its rate-limit-block location yet** if you'd rather leave the
     rate-limit block intact for R8. Two options — pick ONE and be consistent:
     - **(A, recommended)** Delete the `_rl_int` def from the rate-limit block now and let it come from
       settings via the facade import below. Then in R8, `middleware.py` does `from settings import _rl_int`.
     - **(B)** Leave `_rl_int` where it is for now; then settings.py's `env_int` is an independent copy and
       R8 removes the duplicate. (A) is cleaner — no duplicate logic.
   - Keep `import logging`, `import os` in server.py if still used elsewhere (grep first; they are used).

2. **Add the re-export import** near the top of server.py's local-import block (after the R1 imports):
   ```python
   ...
   ```
   **Ordering matters:** this import runs `settings.py` top-to-bottom, which calls `_load_env()`,
   `hmac_keys.validate_startup()`, and the observability init — exactly the side effects that used to run
   inline in server.py, in the same order. Place this import BEFORE any code in server.py that uses `log`
   or `_ADMIN_SECRET` at import time. Since server.py's remaining top-level code runs after the import
   block, this holds.

3. **Remove now-dead imports from server.py ONLY if grep confirms zero remaining uses:** `from dotenv import
   load_dotenv` (now only settings uses it). Check: `grep -n "load_dotenv" backend/server.py`. Leave
   `import hmac_keys` / `import observability` in server.py if anything else references them (grep;
   `hmac_keys` is used by `verify_lca_signature` which is still in server.py until R5 — keep it).

---

## STEP 3 — Gates (from `backend/`)

1. **G3:** `DMRV_DISABLE_DOTENV=1 python -c "import server; from server import _require_secret, _attestation_enforced, _HMAC_SECRET, _rl_int, log; print('ok')"` → `ok`.
2. **G1:** `DMRV_DISABLE_DOTENV=1 python -m pytest -q` → **416 passed, 2 skipped**.
   - Watch these specifically (they exercise settings behavior): the P0-21 "refuse to start without secret"
     test, `test_t26_health_secret.py`, `test_p1_24_cors.py` (reloads server), `test_rate_limit.py`
     (uses `_rl_int` indirectly). All must stay green via the re-exports.

---

## STEP 4 — Commit + tick

- Tracker line: `- [x] **P4.8/R2** — extracted settings.py (env/secrets/flags/consts); server.py ~2600→~2470; 416/2 green`
- Commit:
  ```
  refactor(backend): extract settings.py config layer — server.py ~2600→~2470 LOC (P4.8/R2)

  Pure relocation, no behavior change. Flag readers stay env-live (survive reload).
