...
void main() {
  ...
  test('retake of a smoke_1 capture replaces the row (upsert)', () async {
    ...
    final rows = await db
        .customSelect(
          ...
          variables: [
            Variable.withString(batchUuid),
            Variable.withString('smoke_1'),
          ],
        )
        .get();
  ...
  test(
    'insertPyrolysisTelemetry persists temperature log even if photos incomplete',
    () async {
      ...
      expect(
        saved.length,
        1,
        reason: 'Telemetry row not persisted before photo check.',
      );
      expect(
        saved.first.data['temperature_readings_json'].toString().contains(
          '200.0',
        ),
        isTrue,
      );
