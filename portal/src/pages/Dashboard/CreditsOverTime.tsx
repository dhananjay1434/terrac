 ...
 * doesn't support it yet (see D.1a/D.2's scope note).
 */
export default function CreditsOverTime({
  ...
  return (
    <div className="card" style={{ marginTop: 16 }}>
      <span className="micro">Credits issued over time</span>
      {caption && (
        ...
        {!loading && !error && (
          <BarChart
            ...
            data={(buckets ?? []).map((b) => ({
              ...
              value: b.issued_credit_t_co2e,
            }))}
            formatValue={fmtCredit}
            emptyLabel="No credits issued yet"
            ariaLabel="Credits issued over time"
          />
        )}
      </div>
    </div>
