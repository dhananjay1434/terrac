...
0. **⚠ DECISION GATE (execution finding, 2026-07-27): the compliance gate is MIN-based** (`corroboration.py: PYROLYSIS_MIN_C = 350.0`, `derive_min_temp`, ≥60 samples) — so which probes feed the array directly changes credit eligibility. Module MUST start with `COMPLIANCE_CHANNELS: tuple[str, ...] = ("T1", "T2", "T3")  # interior probes; T4 (base) excluded pending methodology sign-off — see EVOLUTION_RESULT.md "human next" #2` and use ONLY those channels for this array (T4 still stored/charted — context, not compliance). If methodology sign-off has not been recorded in this constant's comment by a human, the flag `telemetry_v2` must stay OFF in production — note this in the task report.
