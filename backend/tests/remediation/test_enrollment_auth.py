...
_ADMIN_SECRET = (
    "test-admin-secret"  # Phase 2: admin auth is distinct from the HMAC pepper
)
...
async def test_register_without_token_rejected_401(client: AsyncClient):
    payload = {"device_id": "device-no-token", "public_key": _B64_KEY}
...
async def test_register_with_valid_token_succeeds(
    client: AsyncClient, admin_mint_token
):
    payload = {"device_id": "device-valid", "public_key": _B64_KEY}
...
async def test_enrollment_token_is_single_use(client: AsyncClient, admin_mint_token):
    # First use
    payload1 = {"device_id": "device-use-1", "public_key": _B64_KEY}
    headers = {"X-Enrollment-Token": admin_mint_token}
    resp1 = await client.post("/api/v1/register", json=payload1, headers=headers)
    assert resp1.status_code == 201

    # Second use
    payload2 = {"device_id": "device-use-2", "public_key": _B64_KEY}
...
@pytest.mark.asyncio
async def test_existing_device_key_not_overwritable(
    ...
    payload1 = {"device_id": "device-dup", "public_key": _B64_KEY}
    resp1 = await client.post(
        "/api/v1/register", json=payload1, headers={"X-Enrollment-Token": "token-a"}
    )
    assert resp1.status_code == 201

    payload2 = {"device_id": "device-dup", "public_key": _B64_KEY}
    resp2 = await client.post(
        "/api/v1/register", json=payload2, headers={"X-Enrollment-Token": "token-b"}
    )
...
async def test_missing_device_id_rejected(client: AsyncClient):
    resp = await client.post(
        "/api/v1/telemetry", json={}, headers={"X-Signature": "dummy"}
    )
...
async def test_unknown_device_id_rejected_403(client: AsyncClient):
    resp = await client.post(
        "/api/v1/telemetry",
        json={},
        headers={"X-Device-Id": "unknown", "X-Signature": "dummy"},
    )
    assert resp.status_code == 403
    assert resp.json()["detail"] == "unknown_device"
...
@pytest.mark.asyncio
async def test_registered_device_valid_signature_accepted(
    client: AsyncClient, admin_mint_token
):
    # Register device with a real Ed25519 public key (Phase 5).
    ...
    headers = {"X-Device-Id": dev_id, "X-Idempotency-Key": op_id, "X-Signature": sig}
