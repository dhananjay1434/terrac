...
@pytest.mark.asyncio
async def test_real_client_biomass_payload_accepted(
...
@pytest.mark.asyncio
async def test_photoless_biomass_accepted_null_sha(
    client: AsyncClient, registered_device
):
    batch_uuid = str(uuid4())
    payload = {
...
@pytest.mark.asyncio
async def test_unknown_extra_field_still_rejected(
    client: AsyncClient, registered_device
):
    batch_uuid = str(uuid4())
    payload = {
