...
async def test_media_anchors_by_explicit_batch_uuid(client: AsyncClient, registered_device, session_factory):
    ...
    with open(tmp_name, "rb") as f:
        resp2 = await client.post("/api/v1/media", files={"file": ("photo.jpg", f, "image/jpeg")}, headers={
    ...
    async with session_factory() as session:
        batch = (await session.execute(select(Batch).where(Batch.batch_uuid == uuid.UUID(b1_uuid)))).scalar_one()
        assert batch.status == "RECEIVED"
        
        media = (await session.execute(select(MediaFile).where(MediaFile.operation_id == "op-media-anch"))).scalar_one()
        assert media.batch_uuid == uuid.UUID(b1_uuid)

async def test_duplicate_photo_hash_does_not_500(client: AsyncClient, registered_device):
    from tests.remediation.crypto_utils import sign_request
    b1_uuid = str(uuid.uuid4())
    ...
    with open(tmp_name, "rb") as f:
        resp1 = await client.post("/api/v1/media", files={"file": ("photo.jpg", f, "image/jpeg")}, headers={
    ...
    with open(tmp_name, "rb") as f:
        resp2 = await client.post("/api/v1/media", files={"file": ("photo2.jpg", f, "image/jpeg")}, headers={
...
async def test_reused_photo_anchors_to_correct_batch(client: AsyncClient, registered_device, session_factory):
    ...
    # batch 1 media
    with open(tmp_name, "rb") as f:
        await client.post("/api/v1/media", files={"file": ("photo.jpg", f, "image/jpeg")}, headers={
    ...
    async with session_factory() as session:
        m1 = (await session.execute(select(MediaFile).where(MediaFile.operation_id == "op-media-ru1"))).scalar_one()
        assert m1.batch_uuid == uuid.UUID(b1_uuid)
        
        m2 = (await session.execute(select(MediaFile).where(MediaFile.operation_id == "op-media-ru2"))).scalar_one()
...
async def test_missing_device_id_on_media_rejected(client: AsyncClient):
    ...
    with open(tmp_name, "rb") as f:
        resp = await client.post("/api/v1/media", files={"file": ("photo.jpg", f, "image/jpeg")}, headers={
