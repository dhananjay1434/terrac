...
async def test_single_sample_temp_rejected(client: AsyncClient, registered_device):
    ...
    headers = {
        ...
        "X-HMAC-Signature": sign_request(dev_id, b64_key, "POST", "/api/v1/batches", "op-single", payload)
    }
    response = await client.post("/api/v1/batches", content=json.dumps(payload).encode("utf-8"), headers=headers)
    assert response.status_code == 400
    assert "missing_qualifying_telemetry_log" in str(response.json()) or "min_recorded_temp_c < 100 C" in str(response.json())
    # Actually if client asserts 200, it passes Pydantic validator, but create_batch fails it.
...
async def test_full_log_required_for_compliant_penalty(client: AsyncClient, registered_device):
    ...
    tel = {
        ...
        "batch_uuid": b1_uuid,
        "temperatureReadingsJson": [210.0] * 65,
    }
    await client.post("/api/v1/telemetry", content=json.dumps(tel).encode("utf-8"), headers={
        "X-Device-Id": dev_id,
        "X-Idempotency-Key": "op-tel-full",
        "X-HMAC-Signature": sign_request(dev_id, b64_key, "POST", "/api/v1/telemetry", "op-tel-full", tel)
    })
    ...
    headers = {
        ...
        "X-HMAC-Signature": sign_request(dev_id, b64_key, "POST", "/api/v1/batches", "op-batch-full", payload)
    }
    response = await client.post("/api/v1/batches", content=json.dumps(payload).encode("utf-8"), headers=headers)
...
async def test_min_temp_derived_from_array_not_scalar(client: AsyncClient, registered_device):
    ...
    tel = {
        "telemetry_uuid": str(uuid.uuid4()),
        "batch_uuid": b1_uuid,
        "temperatureReadingsJson": [180.0] * 60, # Min is 180, which is < 190
    }
    await client.post("/api/v1/telemetry", content=json.dumps(tel).encode("utf-8"), headers={
        "X-Device-Id": dev_id,
        "X-Idempotency-Key": "op-tel-deriv",
        "X-HMAC-Signature": sign_request(dev_id, b64_key, "POST", "/api/v1/telemetry", "op-tel-deriv", tel)
    })
    ...
    headers = {
        ...
        "X-HMAC-Signature": sign_request(dev_id, b64_key, "POST", "/api/v1/batches", "op-batch-deriv", payload)
    }
    response = await client.post("/api/v1/batches", content=json.dumps(payload).encode("utf-8"), headers=headers)
    assert response.status_code == 201
...
async def test_under_reported_transport_flagged(client: AsyncClient, registered_device, session_factory):
    ...
    tel = {
        "telemetry_uuid": str(uuid.uuid4()),
        "batch_uuid": b1_uuid,
        "temperatureReadingsJson": [210.0] * 60,
    }
    await client.post("/api/v1/telemetry", content=json.dumps(tel).encode("utf-8"), headers={
        "X-Device-Id": dev_id,
        "X-Idempotency-Key": "op-tel-transp",
        "X-HMAC-Signature": sign_request(dev_id, b64_key, "POST", "/api/v1/telemetry", "op-tel-transp", tel)
    })
    ...
    headers = {
        ...
        "X-HMAC-Signature": sign_request(dev_id, b64_key, "POST", "/api/v1/batches", "op-batch-transp", payload)
    }
    ...
    app_headers = {
        ...
        "X-HMAC-Signature": sign_request(dev_id, b64_key, "POST", "/api/v1/application", "op-app-transp", app_payload)
    }
