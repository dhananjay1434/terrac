...
async def test_x_mock_location_header_is_not_trusted(
    ...
    headers["X-Signature"] = sign_request(
        dev_id, b64_key, "POST", "/api/v1/batches", "op-mock-1", payload
    )
...
async def test_teleport_between_batches_flagged(client: AsyncClient, registered_device):
    ...
    await client.post(
        "/api/v1/telemetry",
        content=json.dumps(tel1).encode("utf-8"),
        headers={
            "X-Device-Id": dev_id,
            "X-Idempotency-Key": "op-tel-1",
            "X-Signature": sign_request(
                dev_id, b64_key, "POST", "/api/v1/telemetry", "op-tel-1", tel1
            ),
        },
    )
    ...
    await client.post(
        ...
        headers={
            ...
            "X-Signature": sign_request(
                dev_id, b64_key, "POST", "/api/v1/batches", "op-batch-1", payload1
            ),
        },
    )
    ...
    await client.post(
        "/api/v1/telemetry",
        content=json.dumps(tel2).encode("utf-8"),
        headers={
            "X-Device-Id": dev_id,
            "X-Idempotency-Key": "op-tel-2",
            "X-Signature": sign_request(
                dev_id, b64_key, "POST", "/api/v1/telemetry", "op-tel-2", tel2
            ),
        },
    )
    ...
    response = await client.post(
        ...
        headers={
            ...
            "X-Signature": sign_request(
                dev_id, b64_key, "POST", "/api/v1/batches", "op-batch-2", payload2
            ),
        },
    )
